...
%  Manipulation and object recognition are key capabilities in domestic service robots. These tasks have been commonly tackled separately as two independent problems. For object recognition, feature based techniques such as SIFT and SURF are among the most used approaches; more recent works with neural networks-based techniques are growing in popularity. Nevertheless, all these approaches use only visual information (sometimes with depth information) to perform object recognition and thus, they would be unable to distinguish, for example, between a full and an empty can. In this work we propose to estimate the mass of the grasped object using the information provided by the manipulation system. This estimation is achieved using model-based fault reconstruction techniques. We use an Extended Kalman Filter together with a Sliding Mode Observer to achieve both control and mass-estimation. We present several simulation results and discuss the possible error sources. We also discuss how the estimation of the object mass can be useful in domestic service robots. Finally, we state our conclusions and sketch the future work. 
\end{abstract}

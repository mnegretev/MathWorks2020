...
int    center_bits[6]  = {1543, 1909, 1800, 2100, 2051, 2069};
...
void callback_la_torque(std_msgs::Float64MultiArray::Ptr msg)
{
    ...
    for(size_t i=0; i< goal_torque.size(); i++)
        if(msg->data[i] > torque_limit) msg->data[i] = torque_limit;
        else if(msg->data[i] < -torque_limit) msg->data[i] = -torque_limit;
    
    for(size_t i=0; i< goal_torque.size(); i++)
    {
        goal_torque[i] = (uint16_t)(fabs(msg->data[i])*bits_per_N_m[i]);
        if(torque_dir[i]*msg->data[i] < 0)
...
int main(int argc, char **argv)
{
    ...
    std::string str_ids = "0 1 2 3 4 5";
    ...
    while(ros::ok())
    {
        ...
        for(size_t i=0; i< servo_IDs.size(); i++)
            ...
            {
                temp[i] =  groupBulkRead.getData(servo_IDs[i], 36, 2);
                msg_current_pose.data[i] = directions[i]*(center_bits[i]-(int)( temp[i] ))/bits_per_radian;
            }
/*
        std::cout << "Read data: ";
        for(size_t i=0; i< servo_IDs.size(); i++)

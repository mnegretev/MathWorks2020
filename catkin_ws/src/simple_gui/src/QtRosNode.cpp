...
{    
    ros::Rate loop(30);
    subCurrentPose   = n->subscribe("/hardware/la_current_pose", 1, &QtRosNode::callbackCurrentPose, this);
    subEstimatedMass = n->subscribe("/estimated_mass", 1, &QtRosNode::callbackEstimatedMass, this);
    pubGoalPose      = n->advertise<std_msgs::Float64MultiArray>("/hardware/la_goal_pose", 10);
    pubObjectMass    = n->advertise<std_msgs::Float64>("/hardware/la_obj_mass", 10);
